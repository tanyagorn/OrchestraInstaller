...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:doc="urn:emds:docu">
	...
	<xsl:template name ="makeLink">
		...
		<xsl:if test="$elementtype = 'ScenarioElement' ">
			<a><xsl:attribute name="target"><xsl:value-of select="$targetValue"/></xsl:attribute><xsl:attribute name="href"><xsl:value-of select="$hrefValue"/>.xml</xsl:attribute><xsl:value-of disable-output-escaping="yes" select="$textValue"/></a>
		</xsl:if>
