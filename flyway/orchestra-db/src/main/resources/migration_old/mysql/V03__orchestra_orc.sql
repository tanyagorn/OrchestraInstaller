...
CREATE TABLE ORC_DST_TABLE
(
  COL_STRING     VARCHAR(50),
  COL_INT        integer,
  COL_LONG       integer,
  COL_FLOAT      bigint,
  COL_DOUBLE     bigint,
...
CREATE TABLE ORC_EVENT_ACTIONS
(
  EAC_SOURCE       integer,
  EAC_ACTION       integer,
  EAC_DESCRIPTION  VARCHAR(255)
)ENGINE= innodb;
...
CREATE TABLE ORC_EVENT_SOURCE
(
  ESO_SOURCE       integer,
  ESO_DESCRIPTION  VARCHAR(255)
)ENGINE= innodb;
...
CREATE TABLE ORC_EVT_VARIABLES
(
  ID         bigint                         NOT NULL,
  EVV_NAME   VARCHAR(255),
  EVV_VALUE  VARCHAR(255)
...
(
  ...
  RDA_DATE        TIMESTAMP,
  RDA_ACTIVITY    integer,
  RDA_LOCAL_VARS  BLOB
)ENGINE= innodb;

CREATE TABLE ORC_SEC_ROLE
(
...
(
  ...
  USR_EXTPROVIDER  VARCHAR(255)
)ENGINE= innodb;

CREATE TABLE ORC_SEC_USER_ROLES
(
  USER_ID   VARCHAR(255)                  NOT NULL,
...
CREATE TABLE ORC_SEC_GROUP_ROLES
(
...
ALTER TABLE ORC_SEC_ROLE ADD 
  PRIMARY KEY
 (ROLE_NAME);


ALTER TABLE ORC_SEC_USER ADD 
  PRIMARY KEY
 (USER_ID);
...
ALTER TABLE ORC_SEC_USER_ROLES ADD 
  CONSTRAINT FK7064B9567F7925BE 
 FOREIGN KEY (USER_ID) 
 REFERENCES ORC_SEC_USER (USER_ID);

ALTER TABLE ORC_SEC_USER_ROLES ADD 
  CONSTRAINT FK7064B9568D8B9C9B 
 FOREIGN KEY (USR_ROLE) 
 REFERENCES ORC_SEC_ROLE (ROLE_NAME);


ALTER TABLE ORC_SEC_GROUP_ROLES ADD 
  CONSTRAINT FK82945A30AE4B4862 
 FOREIGN KEY (GROUP_ROLE) 
 REFERENCES ORC_SEC_ROLE (ROLE_NAME);

ALTER TABLE ORC_SEC_GROUP_ROLES ADD 
  CONSTRAINT FK82945A30CB319B66 
